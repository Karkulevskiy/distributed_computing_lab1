...
int main(int argc, char *argv[])
{
    ...
    /* Try to insert inserts_in_main keys before starting threads */
    i = attempts = 0;
...
void *Thread_work(void *rank)
{
    ...
    for (i = 0; i < ops_per_thread; i++)
    {
        ...
        {
#ifdef USE_CUSTOM
            rwlock_rdlock(&custom_rwlock);
#else
            pthread_rwlock_rdlock(&rwlock);
#endif
            Member(val);
#ifdef USE_CUSTOM
            rwlock_unlock(&custom_rwlock);
#else
